...
benchmarks =
    ...
    , Bench "mkEqConstraints/congruence-chain-64" 200 $ \i ->
        forceEqConstraints $ mkEqConstraints (congruenceChain 64 i)
...
of all roots, so a pairwise congruence scan grows quadratically with the size.
-}
congruenceChain :: Int -> Int -> [[Path]]
congruenceChain size salt =
    [[path [i], path [i + 1]] | i <- [base .. base + size]]
        ++ [[path [i, 0], path [i, 1]] | i <- [base .. base + size]]
  where
    base = salt `rem` 3
