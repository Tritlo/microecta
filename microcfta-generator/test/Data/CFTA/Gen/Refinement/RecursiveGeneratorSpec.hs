...
compileAtDepth depth = do
    ...
    let annotated = Datatype.annotateDatatype (const (const true, unconstrainedConstraint)) datatype
    LTAGen.compileWith unusedEntailment (LTAGen.fromDatatypeUpToDepth depth annotated)
...
spec = do
    ...
    describe "finite automaton overlap checks" $ do
        ...
        it "keeps the term ranks of a small shared automaton" $ do
            ...
            complete <- LTAGen.compileWith unusedEntailment $ LTAGen.fromAutomatonUpToDepth 5 automaton
  ...
  where
    ...
    compileAutomaton = LTAGen.compileWith unusedEntailment . LTAGen.fromAutomaton

    -- Fold each accepted term into a value as it is selected.
    compileAutomatonWith build =
        LTAGen.compileWith unusedEntailment
