...
import Data.CFTA.Refinement.Expression (refinementFormula, toExpr, true, variable, (.==), (.>=))
    ...
    describe "LTA guard syntax" $ do
        it "states a literal precondition without a phantom predicate child" $ do
            let nonNegative v = v .>= 0
                ...
                term = Tree.Node (LiquidSymbol "divide" true) [Tree.Node (LiquidSymbol "n" (refinementFormula nonNegative)) []]
            ...
            let same = buildGuard $ \left right -> left `isSameTermAs` right
                ...
                differentlyRefinedTerm =
                    Tree.Node
                        ...
                        [Tree.Node (LiquidSymbol "x" true) [], Tree.Node (LiquidSymbol "x" (refinementFormula (\v -> v .== 1))) []]
        ...
        it "renames nested constructor symbols and free refinement names before comparison" $ do
            let annotated symbol name =
                    Tree.Node
                        ...
                        [Tree.Node (LiquidSymbol symbol (refinementFormula (\v -> v .== variable name))) []]
                term =
                    Tree.Node
                        ( LiquidSymbol
                            "context"
            ...
            let term = Tree.Node (LiquidSymbol "pair" true) [Tree.Node (LiquidSymbol "x" true) [], Tree.Node (LiquidSymbol "y" true) []]
                ...
                annotated =
                    Tree.Node
                        ...
                        [ Tree.Node (LiquidSymbol "x" true) []
                        , Tree.Node (LiquidSymbol "y" true) []
                        , Tree.Node (LiquidSymbol "predicate" (refinementFormula (\v -> v .== variable "x"))) []
                        , Tree.Node (LiquidSymbol "predicate" (refinementFormula (\v -> v .== variable "y"))) []
                        ]
        ...
        it "uses the first nonidentity replacement for duplicate formal names" $ do
            let term =
                    Tree.Node
                        ...
                        [ Tree.Node (LiquidSymbol "a" (refinementFormula (\v -> v .== variable "a"))) []
                        , Tree.Node (LiquidSymbol "b" (refinementFormula (\v -> v .== variable "b"))) []
                        , Tree.Node (LiquidSymbol "x" (refinementFormula (\v -> v .== variable "x"))) []
                        , Tree.Node (LiquidSymbol "x" (refinementFormula (\v -> v .== variable "x"))) []
                        ]
                duplicates =
                    Substitute
        ...
        it "applies nested equality scopes from the inner scope to the outer scope" $ do
            let term =
                    Tree.Node
                        ...
                        [ Tree.Node (LiquidSymbol "x" (refinementFormula (\v -> v .== variable "x"))) []
                        , Tree.Node (LiquidSymbol "y" (refinementFormula (\v -> v .== variable "y"))) []
                        , Tree.Node (LiquidSymbol "z" (refinementFormula (\v -> v .== variable "z"))) []
                        ]
                guard =
            ...
            let model :: Fixpoint.Expr
                ...
                term =
                    Tree.Node
                        ...
                        [ Tree.Node (LiquidSymbol "previous" (refinementFormula (\v -> v .== 0))) []
                        , Tree.Node
                            ( LiquidSymbol
                                "command"
                                true
                            ...
                            , Tree.Node (LiquidSymbol "post-state" (refinementFormula (\v -> v .== (toExpr model + 1)))) []
                            ]
                        ]
