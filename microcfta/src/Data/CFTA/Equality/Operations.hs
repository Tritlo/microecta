...
Every path of a class must exist in the term, as in enumeration. A class
whose paths are all missing from the term is not satisfied.
...
equalitiesSatisfied constraints t = all eclassSatisfied (unsafeGetEclasses constraints)
  where
    ...
    eclassSatisfied pec = maybe False allTheSame $ traverse (`getPath` t) $ unPathEClass pec
...
reduceEqConstraints local inherited = fixUnbounded (go local inherited)
  where
    ...
    -- the path or before it.
    meetsConstrainedMu :: [Node symbol constraint] -> Path -> Bool
    meetsConstrainedMu ns (ConsPath index rest) = maybe False (nodeMeetsConstrainedMu rest) (ns !? index)
    meetsConstrainedMu _ EmptyPath = False

    nodeMeetsConstrainedMu :: Path -> Node symbol constraint -> Bool
    nodeMeetsConstrainedMu _ n | numNestedMu n == 0 = False
