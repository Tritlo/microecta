...
-}
module Data.LTA.Gen.Internal.Replay (
    cardinality,
    unrank,
    shrinkRank,
    smallerMembers,
...
compiledSource compiled =
  ...
  where
    outcomes = case Tree.fromIndexedOnDemand $ Tree.Indexed (cardinality compiled) outcomeAt of
        ...
        Left err -> error $ "microlta-generator bug in Data.LTA.Gen.Internal.Replay: invalid compiled source: " <> show err
    outcomeAt rank = case unrank compiled rank of
        ...
        Left err -> error $ "microlta-generator bug in Data.LTA.Gen.Internal.Replay: invalid source rank: " <> show err
