...
-}
denominators :: LTAGen.LTAGen Integer
denominators =
    LTAGen.pool
        [ LTAGen.Refined 0 "zero" (value .==. integer 0)
        , LTAGen.Refined 1 "nonzero" nonZero
        , LTAGen.Refined 2 "input" (value .==. variable "input")
        ]

-- | Reject the zero denominator before evaluating the division.
divisions :: LTAGen.LTAGen (Integer, Integer)
divisions = LTAGen.node "divide" (`requires` nonZero) $ LTAGen.do
