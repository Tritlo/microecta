...
pairs =
    case deriveFTAWith @(Int, Int) $ domain @Int [0, 1] of
        ...
        Right datatype -> FTA.fromDatatypeUpToDepth 1 datatype
...
main = do
    ...
    let leaf = Common.Node [Common.Edge "zero" [], Common.Edge "one" []] :: Common.PlainNode String
        ...
        imported = FTA.fromAutomaton graph
    unless (FTA.cardinality imported == Right 4) $ fail "wrong interned FTA count"
    unless (FTA.unrank imported 3 == Right (Tree.Node "pair" [Tree.Node "one" [], Tree.Node "one" []])) $
