...
"Data.CFTA.Gen.Equality.QuickCheck", which re-exports it:
...
bind chooses the operation family (keyed by 'Data.CFTA.Gen.Equality.Sig'), the
remaining binds choose one argument per signature component in order, and the
block builds exactly the 'Data.CFTA.Gen.Equality.apply' join:
