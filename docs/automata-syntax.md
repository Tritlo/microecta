...
| LTA | These refinements logically imply one another. | `LTAGen.node "label" $ LTAGen.do ...`, with `satisfying` and `guarded` |
...
LTA dependencies need the solver. A refinement is a Haskell function of the
value, as in `\v -> v ./= 0`, and there are two places to state one. A
condition on one child goes where the child is drawn:
...
A relation between children is the contract of the node, a function with one
term for each child:
...
reads =
  LTAGen.guarded "read-at" inBounds $ LTAGen.do
    size  <- sizes
    index <- indexes
    LTAGen.pure (size, index)
```

The solver proves each conjunct of a contract separately. For a conjunct, it
assumes the refinement of each child that the conjunct names, then proves the
conjunct. `elements` infers the refinement `\v -> v .== literal x`
of each integer, so a condition decides each value exactly; `pool` takes values
with hand-written refinements, which can say less than the value.

This split is deliberate. A name bound in the do-block has one Haskell type.
The condition is about a child's refinement, a term of the logic, while the
result, such as `12 `div` d`, needs the Haskell value. One name cannot be both
without a wrapper type in every generated value. So a condition on one child
needs no name at all, and a contract names the children as a function
signature names its parameters, while the do-block binds their values.

The paper's encodings, where a function type is a subtree of the term, still
use positional guards. `refinedNode` and `refinedNodeByRoots` take them, and
they also give the constructor its result refinement:
...
index-taking `argument 0`, extra predicate children, a guard lambda that names
the children again next to the do-block, and a quasiquoter in the paper's
notation. Raw paths expose LTA machinery, an index or a second name separates a
child from its binding, predicate children alter the term only to satisfy the
API, and a quasiquoter needs Template Haskell and a Haskell parser.


## Compile once and retain an escape hatch

`LTAGen.compile language` preserves source order, occurrence weights, and
semantic pool shrinking. It starts Z3, and `compileWith solver language` uses a
given solver instead. It groups candidates by the observations their guards
...
refinements as errors; `compile` fails with the `explain` text of the error,
and `compileWith` returns it. Use `validOutcomes`
