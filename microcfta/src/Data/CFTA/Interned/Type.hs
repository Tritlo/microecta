...
nodeIds :: IdSupply
nodeIds = unsafePerformIO newIdSupply
{-# NOINLINE nodeIds #-}
...
internNode = intern (selectCache @symbol @constraint nodeCaches (freshCacheWith nodeIds)) identifyNode
...
edgeIds :: IdSupply
edgeIds = unsafePerformIO newIdSupply
{-# NOINLINE edgeIds #-}
...
internEdge = intern (selectCache @symbol @constraint edgeCaches (freshCacheWith edgeIds)) InternedEdge
...
environment, except a subgraph without free variables, which it returns
unchanged. See @intersect@ for an operation that can avoid an environment
...
substitutionPlan inputNode = memoTypeableWith @symbol @constraint genericSubstitutionPlanCache onNode inputNode
  where
    onNode :: Node symbol constraint -> SubstitutionPlan symbol constraint (Node symbol constraint)
    onNode n = SubstitutionPlan $
        case n of
            -- A node without free variables has nothing to substitute.
            _ | Set.null (freeVars n) -> const n
...
edgeSubstitutionPlan inputEdge = memoTypeableWith @symbol @constraint genericEdgeSubstitutionPlanCache onEdge inputEdge
  where
