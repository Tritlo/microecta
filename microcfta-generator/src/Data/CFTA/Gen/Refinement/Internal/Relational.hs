...
-}
module Data.CFTA.Gen.Refinement.Internal.Relational (
    ObservationKey,
    RelationalValue,
    RecipeGroups,
    acceptedAlphabet,
...
import Data.CFTA.Gen.Refinement.Internal.Replay (cardinality, unrank)
...
import Data.CFTA.Gen.Refinement.Internal.Witness (Witness (..), cacheEntailment, termWitness, witnessTerm)
...
groupAt key groups =
    case Map.lookup key $ acceptedGroups groups of
        Just info -> info
        Nothing -> error "microcfta-generator bug in Data.CFTA.Gen.Refinement.Internal.Relational: missing source group"
  ...
  where
    ...
    memberAt sources index =
        case Source.select sources index >>= either (const Nothing) Just . (unrank compiled) of
            ...
            Nothing -> error "microcfta-generator bug in Data.CFTA.Gen.Refinement.Internal.Relational: invalid imported group rank"
...
refinementForChildren (ComputedRefinement _) _ =
    error
        "microcfta-generator bug in Data.CFTA.Gen.Refinement.Internal.Relational.refinementForChildren: a value-computed refinement reached relational compilation"
...
observationKeyRoot (ObservationKey observations) =
    case Map.lookup (path []) observations of
        ...
        Nothing ->
            error
                "microcfta-generator bug in Data.CFTA.Gen.Refinement.Internal.Relational.compileRelational: child group has no root observation"
...
relationalGeneratedAt generator rank =
    case ECTA.unrank generator rank of
        Left err ->
            error $
                "microcfta-generator bug in Data.CFTA.Gen.Refinement.Internal.Relational.compileRelational: invalid retained rank: "
                    <> show err
