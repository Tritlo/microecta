...
data Plan a where
    ...
    PlanShared :: !Integer -> RankDecoder a -> Plan a -> Plan a
    -- | Map decoded values.
...
compileRank (PlanShared _ (SmallDecoder _ decode) _) = decode
compileRank (PlanShared _ (LargeDecoder _ decode) _) = decode . toInteger
...
compileLargeRank (PlanShared _ (SmallDecoder _ decode) _) = decode . fromInteger
compileLargeRank (PlanShared _ (LargeDecoder _ decode) _) = decode
