...
data PathTrieCommand
    = PathTrieZipperAscend Int
...
spec = do
    ...
    describe "path trie zipper" $ do
        ...
        it "a sequence of path trie zipper ascends/descends followed by its reverse yields the identity" $ do
            property $ \actions pt ->
                (zipperCurPathTrie $ foldr applyPathTrieCommand (pathTrieToZipper pt) (reverse (map invertPathTrieCommand actions) ++ actions))
                    == pt
