...
{- | Annotate constructors by name, and give each other constructor a default.

A name that no constructor of the grammar has gives 'UnknownConstructors', so a
misspelt name cannot fall back to the default. Constructors of different types
that have the same name get the same annotation.
-}
annotateConstructors ::
    constraint -> [(String, constraint)] -> TypedFTA old a -> Either DeriveError (TypedFTA constraint a)
annotateConstructors fallback named datatype
    | null unknown = Right $ annotateDatatype annotate datatype
  ...
  where
    ...
    annotate constructor = Map.findWithDefault fallback (constructorName constructor) table
...
data DeriveError
    ...
    | -- | Annotation names that no constructor of the grammar has.
      UnknownConstructors ![String]
    deriving (Eq, Show)
